...
export default function Skills() {
  return (
    <section id="المهارات" className="section-padding bg-linen-100">
      <div className="container-page">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {skills.map(({ icon: Icon, text }) => (
            <div
              key={text}
              className="flex items-start gap-4 rounded-2xl border border-linen-300 bg-white p-5 shadow-card"
            >
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-saffron-100 text-saffron-600">
                <Icon size={20} strokeWidth={1.8} />
              </span>
              <p className="pt-1 text-base leading-7 text-ash-700">{text}</p>
            </div>
          ))}
        </div>
      </div>
